...
namespace base
{
	...
	{
		if (pLuaFacade == nullptr)
			throw std::logic_error("invalid namespace arg");
	}
	...
	void CNamespaceBinder::begin(const char* szName)
	{
		...
		SAFE_DELETE_ARRAY(this->m_szNamespaceName);
		size_t nLen = crt::strnlen(szName, _TRUNCATE);
		this->m_szNamespaceName = new char[nLen + 1];
		crt::strcpy(this->m_szNamespaceName, nLen + 1, szName);
	}
