...
#define MARK 6
	...
	const char* keystring(lua_State* pL, int32_t index, char* buffer, size_t size)
	{
		...
		case LUA_TNUMBER:
			base::crt::snprintf(buffer, size, "[%lg]", lua_tonumber(pL, index));
			break;
		case LUA_TBOOLEAN:
			base::crt::snprintf(buffer, size, "[%s]", lua_toboolean(pL, index) ? "true" : "false");
			break;
		case LUA_TNIL:
			base::crt::snprintf(buffer, size, "[nil]");
			break;
		default:
			base::crt::snprintf(buffer, size, "[%s:%p]", lua_typename(pL, t), lua_topointer(pL, index));
			break;
		}
		return buffer;
	}
	...
	void mark_function(lua_State* pL, lua_State* pNewL, const void* parent, const char* desc)
	{
		...
		else
		{
			...
			char tmp[16];
			base::crt::snprintf(tmp, _countof(tmp), ":%d", ar.linedefined);
	...
	void mark_thread(lua_State* pL, lua_State* pNewL, const void* parent, const char* desc)
	{
		...
		while (lua_getstack(pcL, level, &ar))
		{
			...
			{
				char tmp[16];
				base::crt::snprintf(tmp, _countof(tmp), ":%d ", ar.currentline);
				luaL_addstring(&b, tmp);
			}
			...
			for (int32_t j = 1; j > -1; j -= 2)
			{
				for (int32_t i = j;; i += j)
				{
					...
					if (name == NULL)
						break;
					base::crt::snprintf(tmp, sizeof(tmp), "%s : %s:%d", name, ar.short_src, ar.currentline);
	...
	{
		char tmp[32];
		size_t l = base::crt::snprintf(tmp, _countof(tmp), "%p : ", parent);
