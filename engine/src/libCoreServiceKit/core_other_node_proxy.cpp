...
namespace core
{
	...
	void CCoreOtherNodeProxy::addNodeProxyInfo(const SNodeBaseInfo& sNodeBaseInfo, const std::vector<SServiceBaseInfo>& vecServiceBaseInfo, bool bMaster)
	{
		...
		sNodeProxyInfo.pTicker->setCallback([&sNodeProxyInfo](uint64_t nContext)
		{
			...
			CBaseApp::Inst()->getBaseConnectionMgr()->connect(sNodeProxyInfo.sNodeBaseInfo.szHost, sNodeProxyInfo.sNodeBaseInfo.nPort, eBCT_ConnectionToOtherNode, szBuf, sNodeInfo.sNodeBaseInfo.nSendBufSize, sNodeInfo.sNodeBaseInfo.nRecvBufSize);
		});
	...
	void CCoreOtherNodeProxy::delNodeProxyInfo(uint16_t nID)
	{
		...
		if (sNodeProxyInfo.pCoreConnectionOtherNode != nullptr)
			sNodeProxyInfo.pCoreConnectionOtherNode->shutdown(base::eNCCT_Force, "del node");
	...
	void CCoreOtherNodeProxy::delCoreConnectionOtherNodeByNodeID(uint16_t nNodeID)
	{
		...
		for (size_t i = 0; i < sNodeProxyInfo.vecServiceBaseInfo.size(); ++i)
		{
			...
			sServiceProxyInfo.pCoreConnectionOtherNode = nullptr;
		}

		sNodeProxyInfo.pCoreConnectionOtherNode = nullptr;
	}

	bool CCoreOtherNodeProxy::addCoreConnectionOtherNodeByNodeID(uint16_t nNodeID, CCoreConnectionOtherNode* pCoreConnectionOtherNode)
	{
		DebugAstEx(pCoreConnectionOtherNode != nullptr, false);

		auto iter = this->m_mapNodeProxyInfo.find(nNodeID);
		if (iter == this->m_mapNodeProxyInfo.end())
		{
			PrintWarning("unknwon node node_id: %d remote_addr: %s %d", nNodeID, pCoreConnectionOtherNode->getRemoteAddr().szHost, pCoreConnectionOtherNode->getRemoteAddr().nPort);
			return false;
		}
		...
		for (size_t i = 0; i < sNodeProxyInfo.vecServiceBaseInfo.size(); ++i)
		{
			...
			SServiceProxyInfo& sServiceProxyInfo = iter->second;
			sServiceProxyInfo.pCoreConnectionOtherNode = pCoreConnectionOtherNode;
		}

		sNodeProxyInfo.pCoreConnectionOtherNode = pCoreConnectionOtherNode;

		return true;
	}

	CCoreConnectionOtherNode* CCoreOtherNodeProxy::getCoreConnectionOtherNodeByNodeID(uint16_t nNodeID) const
	{
		auto iter = this->m_mapNodeProxyInfo.find(nNodeID);
		if (iter == this->m_mapNodeProxyInfo.end())
			return nullptr;

		return iter->second.pCoreConnectionOtherNode;
	}

	CCoreConnectionOtherNode* CCoreOtherNodeProxy::getCoreConnectionOtherNodeByServiceID(uint16_t nServiceID) const
	{
		auto iter = this->m_mapServiceProxyInfo.find(nServiceID);
		if (iter == this->m_mapServiceProxyInfo.end())
			return nullptr;
