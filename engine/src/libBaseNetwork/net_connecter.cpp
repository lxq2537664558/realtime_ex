...
namespace base
{
	void CNetConnecter::printInfo(const char* szFormat, ...)
	{
		...
		PrintInfo("{} connect state: {} flag：{} local addr: {} {} remote addr: {} {} socket_id: {} error code: {} send_index: {} send_count: {}", szBuf, this->m_eConnecterState, this->m_nFlag, this->getLocalAddr().szHost, this->getLocalAddr().nPort, this->getRemoteAddr().szHost, this->getRemoteAddr().nPort, this->getSocketID(), getLastError(), this->m_nSendConnecterIndex, this->m_pNetEventLoop->getSendConnecterCount());
	}
