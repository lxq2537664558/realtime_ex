...
namespace base
{
	...
	uint32_t crc32(const char* szBuf, uint32_t nBufSize)
	{
		if (nullptr == szBuf)
			return 0;

		uint32_t nRet = 0;//CRC32校验码初值
		uint32_t nTableIndex = 0;

		while (nBufSize--)
		{
			nTableIndex = (nRet >> 24) ^ *szBuf++;
			nRet = (nRet << 8) ^ s_zCrctable[nTableIndex];
		}
