...
namespace base
{
	int32_t base64_encode(const char* szData, uint32_t nLen, char* szBuf, uint32_t nBufSize)
	{
	...
	int32_t base64_decode(const char* szData, uint32_t nLen, char* szBuf, uint32_t nBufSize)
	{
		if (nullptr == szBuf || szData == nullptr)
			return -1;
