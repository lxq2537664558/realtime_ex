...
namespace core
{
	...
	std::string CLoadBalanceRandPolicy::select(uint32_t nMessageID, bool bGate, uint64_t nContext)
	{
		const std::vector<std::string>& vecServiceName = CBaseApp::Inst()->getMessageServiceName(nMessageID, bGate);
