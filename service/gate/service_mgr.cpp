...
void CServiceMgr::addServiceConnection(CConnectionFromService* pServiceConnection)
{
	...
	auto iter = this->m_mapServiceConnection.find(pServiceConnection->getServiceName());
	DebugAst(iter == this->m_mapServiceConnection.end());

	this->m_mapServiceConnection[pServiceConnection->getServiceName()] = pServiceConnection;
}
